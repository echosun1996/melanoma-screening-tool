...
class ViT_ImageNet(nn.Module):
    ...
    def forward(self, x):
        """Modified forward to handle different input sizes"""
        B, C, H, W = x.shape
        ...
        # No need to handle pos_embed resize here since we did it in model init
...
def create_vit(args):
    ...
    model = ViT_ImageNet(
        ...
        drop_rate=args.imaging_dropout_rate,
        pretrained=args.imaging_pretrained,
        pretrained_path=args.imaging_pretrained_path if hasattr(args, 'imaging_pretrained_path') else None,
